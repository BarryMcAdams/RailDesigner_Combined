...
namespace RailCreator
{
    public static class PostGenerator
    {
        public static List<Point3d> CalculatePostPositions(Polyline polyline, RailingDesign design, BlockTableRecord btr, Transaction tr)
        {
            ...
            {
                // Calculate post length based on design (kept from original code)
                double postLength = design.RailHeight;
                switch (design.MountingType)
                {
                    case "Core-Drilled":
                ...
                // Find the parameter at the closest point to handle floating point inaccuracies
                double param = polyline.GetParameterAtPoint(polyline.GetClosestPointTo(position, false));
                // Clamp param to be within polyline's valid range
                param = Math.Max(polyline.StartParam, Math.Min(polyline.EndParam, param));

                // Handle potential zero tangent at vertices or ends
                Vector3d tangent = polyline.GetFirstDerivative(param);

                // If tangent is near zero (at a sharp vertex or start/end), try finding a better direction
                if (tangent.Length < geometryEpsilon)
                {
                    ...
                    for (int i = 0; i < polyline.NumberOfVertices - 1; i++)
                    {
                        ...
                        // Check if the point is on this segment (within geometry tolerance)
                    ...
                    if (segmentIndex != -1)
                    {
                        // Use the direction of the segment the point is on
